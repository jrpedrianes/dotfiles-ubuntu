...
### Claude Code settings (`home/dot_claude/settings.json.tmpl`)
Templated so absolute paths resolve at apply-time via `{{ .chezmoi.homeDir }}` instead of being hardcoded.

Its `SessionStart` hook calls `~/.claude/hooks/herdr-agent-state.sh`, a script generated by
`herdr integration install claude` — **not versioned here**. On a fresh machine run it manually after
`mise install` (herdr is pinned in `home/dot_config/mise/config.toml`); until then the hook points at a
missing file. Reinstalling the integration rewrites `~/.claude/settings.json` with the expanded path;
chezmoi is the source of truth, so a later `chezmoi apply` restores it — but if herdr ever changes the
block's shape, re-sync the template by hand.
